...
def check_if_player_close_to_ball(px, py, bx, by):
    if abs(px - bx) > 1:
        return False
    if abs(py - by) > 1:
        return False
...
class Football(Problem):
    def __init__(self, initial):
        ...
        self.opponents_adjacent = [(2, 2), (2, 3), (2, 4), (3, 4), (4, 4), (4, 5), (5, 5), (6, 5), (6, 4), (6, 3),
                                   (5, 3), (4, 3), (4, 2), (3, 2)]
        self.goal = [(7, 2), (7, 3)]
    ...
    def successor(self, state):
        ...
        actions_ball = (
            "Turni topka gore", "Turni topka dolu", "Turni topka desno", "Turni topka gore-desno",
            "Turni topka dolu-desno")
        directions = ((0, 1), (0, -1), (1, 0), (1, 1), (1, -1))

        for action, direction in zip(actions_player, directions):
            rez = self.move_player(state, direction)
            if rez is not None:
                successor_actions[action] = rez

        for action, direction in zip(actions_ball, directions):

            rez = self.move_ball(state, direction)

            if rez is not None:
                successor_actions[action] = rez

        return successor_actions
    ...
    def move_ball(self, state, direction):
        player, ball = state
    ...
    def is_valid_state(self, state):
        ...
        if ball in self.opponents_adjacent or ball in self.opponents:
            return False
    ...
    def h(self, node):
        ...
        distance1 = math.sqrt((gx1 - bx) ** 2 + (gy1 - by) ** 2)
        distance2 = math.sqrt((gx2 - bx) ** 2 + (gy2 - by) ** 2)
