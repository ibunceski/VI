...
class Hanoi(Problem):
    ...
    def successor(self, state):
        ...
        for i, tower in enumerate(state):
            ...
            tower.append(top_elem)

            for j, other_tower in enumerate(state):
                if i == j:
                    continue

                other_tower = list(other_tower)

                if len(other_tower) == 0 or (other_tower[len(other_tower) - 1] >= top_elem):
                    other_tower.append(top_elem)
                    other_tower = tuple(other_tower)
                    tower.pop()
                    new_state = list(state)
                    new_state[i] = tuple(tower)
                    new_state[j] = other_tower
                    tower.append(top_elem)

                    successors[f'MOVE TOP BLOCK FROM PILLAR {i + 1} TO PILLAR {j + 1}'] = tuple(new_state)
...
if __name__ == "__main__":
    ...
    initial = generate_tuple(str_initial)
    goal = generate_tuple(str_goal)
