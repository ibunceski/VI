...
class Squares(Problem):
    ...
    def successor(self, state):
        ...
        for i in range(5):
            actions = (
            f"Pomesti kvadratche {i + 1} gore", f"Pomesti kvadratche {i + 1} dolu", f"Pomesti kvadratche {i + 1} levo",
            f"Pomesti kvadratche {i + 1} desno")
            directions = ((0, 1), (0, -1), (-1, 0), (1, 0))

            for act, dir in zip(actions, directions):
                rez = self.move(state, dir, i)
                if rez is not None:
                    successors[act] = rez
    ...
    def move(self, state, dir, i):
        to_move = state[i]
        x, y = to_move[0] + dir[0], to_move[1] + dir[1]
...
if __name__ == "__main__":
    ...
    problem = Squares(squares, ((0, 4), (1, 3), (2, 2), (3, 1), (4, 0)))
