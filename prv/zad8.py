...
class Checkers(Problem):
    def __init__(self, initial, n, walls):
    ...
    def goal_test(self, state):
        ball = state[0]
        return len(state) == 1 and ball == (self.n // 2, self.n - 1)
    ...
    def successor(self, state):
        ...
        for i, ball in enumerate(state):
            ...
            for act, dir in zip(actions, directions):
                remove_position = (bx + dir[0] // 2, by + dir[1] // 2)
                new_bx, new_by = bx + dir[0], by + dir[1]

                if (new_bx, new_by) in state_set or remove_position not in state_set:
                    continue

                new_state = list(state)
                new_state[i] = (new_bx, new_by)

                new_state.remove(remove_position)

                successors[f'{act}(x={bx},y={by})'] = tuple(new_state)
    ...
    def valid(self, bx, by):
        if not (0 <= bx < self.n and 0 <= by < self.n):
            return False
...
if __name__ == "__main__":
    ...
    problem = Checkers(balls, n, walls)
