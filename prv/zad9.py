...
class Puzzle(Problem):
    ...
    def goal_test(self, state):
        return [elem for j in range(0, self.n) for elem in state[j]] == [1] * (self.n * self.n)
    ...
    def successor(self, state):
        ...
        for i in range(self.n):
            for j in range(self.n):
                ...
                color_positions = ((i, j), (i + 0, j + 1), (i + 1, j + 0), (i - 1, j + 0), (i + 0, j - 1))

                for new_x, new_y in color_positions:
                    if not (0 <= new_x < self.n and 0 <= new_y < self.n):
                        continue

                    new_state[new_x] = list(new_state[new_x])
                    new_state[new_x][new_y] = abs(new_state[new_x][new_y] - 1)

                successors[f'x: {i}, y: {j}'] = tuple(map(tuple,new_state))
...
if __name__ == "__main__":
    ...
    initial = [matrix[i:i + n] for i in range(0, len(matrix), n)]

    for i, elem in enumerate(initial):
        initial[i] = tuple(elem)

    problem = Puzzle(tuple(initial), n)
