...
if __name__ == '__main__':
    ...
    train_set = [row for row in dataset0[:int(x * len(dataset0))]] \
                + [row for row in dataset1[:int(x * len(dataset1))]]
    train_x = [row[:-1] for row in train_set]
    train_y = [row[-1] for row in train_set]

    test_set = [row for row in dataset0[int(x * len(dataset0)):]] \
               + [row for row in dataset1[int(x * len(dataset1)):]]
    ...
    classifierForest = RandomForestClassifier(n_estimators=4, criterion="entropy", random_state=0)
    classifierMLP = MLPClassifier(hidden_layer_sizes=(10), activation="relu", learning_rate_init=0.001, random_state=0)

    classifiers = [classifierNB, classifierTree, classifierForest, classifierMLP]
    accuracies = []
    ...
    best_classifier = accuracies.index(max(accuracies))
    classifier_names = ["Naive Bayes", "Decision Tree", "Random Forest", "MLP"]

    print(f'Najgolema tocnost ima klasifikatorot {classifier_names[best_classifier]}')
    ...
    for row in test_x:
        ...
        if one > zero:
            predictions_group.append(1)
        else:
            predictions_group.append(0)

    recall = recall_score(test_y, predictions_group)
